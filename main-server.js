...
app.post('/registrar', (req, res) => {
  ...
  const query = 'INSERT INTO usuarios (user, password, email) VALUES (?, ?, ?)';
  banco.conn.query(query, [usuario, senha, email], (err, results) => {
    if (err) {
      // console.log(err)
...
app.post('/login', (req, res, next) => {
  ...
  const query = 'SELECT usuarioID FROM usuarios WHERE user=? and password=?';

  banco.conn.query(query, [usuarioRequest, senhaRequest], function (error, results) {
    ...
    if (error) {
      ...
      //auth ok
      const id = results[0].usuarioID //esse id viria do banco de dados
      // console.log(id);
      // console.log(results)
      if (id) {
